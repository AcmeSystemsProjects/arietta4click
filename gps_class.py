...
class gps():
	...
	def parseGGA(self, fields):
		...
		# RMC has 12 fields
		if len(fields) < 15: return
		
		# MsgId = fields[0]
		self.data['UtcTime'] = fields[1]
		self.data['Latitude'] = self.toDecimalDegrees(fields[2])
		self.data['NsIndicator'] = fields[3]
		self.data['Longitude'] = self.toDecimalDegrees(fields[4])
		self.data['EwIndicator'] = fields[5]
		self.data['PosFixIndicator'] = self._int(fields[6])
		self.data['SatellitesUsed'] = self._int(fields[7])
		self.data['HDOP'] = self._float(fields[8])
		self.data['MSLAltitude'] = self._float(fields[9])
		self.data['MSLAunits'] = fields[10]
		self.data['GeoidSep'] = self._float(fields[11])
		self.data['GSunits'] = fields[12]
		self.data['AgeOrDiffCorr'] = self._float(fields[13])
		self.data['DiffrefStationId'] = fields[14]
	...
	def parseGSV(self, fields):
		...
		*73
		...
		# the GSV message spans over n sentences
		n = self._int(fields[1]) # number of sentences
		m = self._int(fields[2]) # sentence m of n
	...
	def parseLine(self, line):
		...
		if len(line) == 0:
			...
			return (None,None)
		#
		# check front and back  sep
		#
		if line[0] != '$':
			print ">>>>>>> Bad front sep !!!!! [%s]" % line[0]
			return (None,None)
		
		if line[-3] != '*':
			print ">>>>>>> Bad trailing sep !!!!! [%s]" % line[-1]
			return (None,None)
		
		# Validate the sentence using the checksum
		if self.calcCheckSum(line) != int(line[-2:], 16) :
			print ">>>>>>> Wrong checksum: [%d] [%s]" % (int(line[-2:], 16), line[-2:])
			return (None,None)
		...
		try:
			...
			print ">>>>>>> Unknown message !!!!! [%s]" % line[3:6]
			return (None,None)
		...
		if parseFunc != None:
			...
			print ">>>>>>> Not yet implemented !!!!! [%s]" % line[3:6]
		
		# Return the type of sentence that was parsed
		return (line[2], line[3:6])
